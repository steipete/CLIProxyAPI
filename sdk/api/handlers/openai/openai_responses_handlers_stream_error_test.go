...
type oversizedPreambleResponsesStreamExecutor struct {
	prematureResponsesStreamExecutor
}

func (*oversizedPreambleResponsesStreamExecutor) Identifier() string {
	return "oversized-responses-preamble"
}

func (*oversizedPreambleResponsesStreamExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	chunks := make(chan coreexecutor.StreamChunk, 2)
	chunks <- coreexecutor.StreamChunk{Payload: []byte(": " + strings.Repeat("x", 2*1024*1024) + "\n\n")}
...
func TestSanitizeResponsesStreamErrorMessageRedactsCredentialVariants(t *testing.T) {
	tests := []struct {
		name   string
		format string
		secret string
	}{
		{name: "query key", format: "https://example.test/v1?key=%s&alt=sse", secret: "query-secret-value-123456"},
		{name: "spaced api key", format: "api key: %s", secret: "spaced-secret-value-123456"},
		{name: "google api header", format: "X-Goog-Api-Key: %s", secret: "google-secret-value-123456"},
		{name: "refresh token", format: "refresh_token=%s", secret: "refresh-secret-value-123456"},
		{name: "bearer authorization", format: "Authorization: Bearer %s", secret: "bearer-secret-value-123456"},
		{name: "basic authorization", format: "Proxy-Authorization: Basic %s", secret: "YmFzaWMtc2VjcmV0LXZhbHVlLTEyMzQ1Ng=="},
		{name: "url password", format: "https://user:%s@example.test/v1/responses", secret: "url-password-secret-123456"},
		{name: "project token signature", format: "provider rejected %s", secret: "sk-proj-abcdefghijklmnopqrstuvwxyz1234567890"},
		{name: "google token signature", format: "provider rejected %s", secret: "AIzaSyABCDEFGHIJKLMNOPQRSTUVWXYZ123456"},
		{name: "double quoted api key with spaces", format: `api_key="%s"`, secret: "first-secret middle-secret last-secret"},
		{name: "single quoted access token with spaces", format: `access_token='%s'`, secret: "first-token middle-token last-token"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			raw := fmt.Sprintf(tt.format, tt.secret)
			...
			if got == nil || got.Error == nil {
				t.Fatal("expected sanitized stream error")
			}
			if strings.Contains(got.Error.Error(), tt.secret) {
				t.Fatalf("sanitized error leaked credential: %q", got.Error.Error())
			}
			for _, part := range strings.Fields(tt.secret) {
				if strings.Contains(got.Error.Error(), part) {
					t.Fatalf("sanitized error leaked part of a quoted credential: %q", got.Error.Error())
				}
			}
			if !strings.Contains(got.Error.Error(), "[REDACTED]") {
				t.Fatalf("sanitized error omitted a redaction marker: %q", got.Error.Error())
			}
		})
	}
}

func TestRedactResponsesStreamErrorTextRedactsCompleteCookieHeaders(t *testing.T) {
	for _, header := range []string{
		"Cookie: session=first-cookie-secret; csrf=second-cookie-secret X-Trace: preserved",
		"Set-Cookie: session=first-cookie-secret; refresh=second-cookie-secret; Path=/ X-Trace: preserved",
		`Cookie: "session=first-cookie-secret; csrf=second-cookie-secret" X-Trace: preserved`,
	} {
		got := redactResponsesStreamErrorText(header)
		if strings.Contains(got, "first-cookie-secret") || strings.Contains(got, "second-cookie-secret") {
			t.Fatalf("cookie header leaked credentials: %q", got)
		}
		if !strings.Contains(got, "X-Trace: preserved") {
			t.Fatalf("cookie redaction swallowed an unrelated header: %q", got)
		}
	}
}

func TestRedactResponsesStreamErrorTextBoundsInputAndClosesTruncatedQuotedSecret(t *testing.T) {
	secretPrefix := "first-secret middle-secret last-secret"
	raw := `api_key="` + secretPrefix + strings.Repeat(" confidential", responsesStreamErrorInputLimit) + `" trailing`
	got := redactResponsesStreamErrorText(raw)
	if len(got) > responsesStreamErrorInputLimit+32 {
		t.Fatalf("provider error input remained unbounded: %d bytes", len(got))
	}
	for _, part := range strings.Fields(secretPrefix) {
		if strings.Contains(got, part) {
			t.Fatalf("quoted credential leaked at truncation boundary: %q", got)
		}
	}
	if !strings.Contains(got, "[REDACTED]") {
		t.Fatalf("bounded error omitted credential redaction: %q", got)
	}
}

func TestResponsesHandlerRejectsOversizedPreambleBeforeFirstDataFrame(t *testing.T) {
	gin.SetMode(gin.TestMode)

	executor := &oversizedPreambleResponsesStreamExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	auth := &coreauth.Auth{ID: "oversized-responses-preamble-auth", Provider: executor.Identifier(), Status: coreauth.StatusActive}
	if _, errRegister := manager.Register(context.Background(), auth); errRegister != nil {
		t.Fatalf("register auth: %v", errRegister)
	}
	registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "gpt-4o"}})
	...
	router := gin.New()
	router.POST("/v1/responses", h.Responses)
	request := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"gpt-4o","input":"hi","stream":true}`))
	request.Header.Set("Content-Type", "application/json")
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)

	if recorder.Code != http.StatusBadGateway {
		t.Fatalf("oversized stream preamble status = %d, want %d", recorder.Code, http.StatusBadGateway)
	}
	if !strings.Contains(recorder.Body.String(), "before first data frame") {
		t.Fatalf("oversized preamble error = %q", recorder.Body.String())
	}
	if recorder.Header().Get("Content-Type") == "text/event-stream" {
		t.Fatal("oversized preamble committed SSE headers before any data frame")
	}
}
