...
func (f *responsesSSEFramer) WriteChunk(w io.Writer, chunk []byte) {
	...
	if responsesSSENeedsLineBreak(f.pending, chunk) {
		if f.dataFrames > 0 && len(f.pending) >= maxResponsesPendingFrameBytes {
			f.rejectOversizedPendingFrame(w)
			return
		}
		f.pending = append(f.pending, '\n')
	}
	if f.dataFrames > 0 && len(chunk) > maxResponsesPendingFrameBytes-len(f.pending) {
		f.rejectOversizedPendingFrame(w)
		return
	}
	...
	canEmit := responsesSSECanEmitWithoutDelimiter(f.pending)
	if !canEmit && f.dataFrames > 0 && responsesSSECanFlushWithoutDelimiter(f.pending) {
		if payload, ok := responsesSSEDataPayload(f.pending); ok {
			canEmit = gjson.GetBytes(payload, "type").String() != ""
		}
	}
	if len(f.pending) == 0 || !canEmit {
		return
	}
	f.writeFrame(w, f.pending)
	f.pending = f.pending[:0]
...
func (f *responsesSSEFramer) rejectOversizedPendingFrame(w io.Writer) {
	f.pending = nil
	payload := []byte(`data: {"type":"error","status":502,"error":{"message":"upstream SSE frame exceeds maximum pending size"}}`)
	f.writeFrame(w, payload)
}
...
func (h *OpenAIResponsesAPIHandler) handleStreamingResponse(c *gin.Context, rawJSON []byte) {
	...
	for {
		...
		case chunk, ok := <-dataChan:
			...
			if framer.dataFrames == 0 {
				if initialOutput.Len()+len(framer.pending) > maxResponsesInitialPreambleBytes {
					errMsg := &interfaces.ErrorMessage{
						StatusCode: http.StatusBadGateway,
						Error:      fmt.Errorf("upstream stream exceeded %d-byte preamble limit before first data frame", maxResponsesInitialPreambleBytes),
					}
					h.LoggingAPIResponseError(context.WithValue(context.Background(), "gin", c), errMsg)
					h.WriteErrorResponse(c, errMsg)
					cliCancel(errMsg.Error)
...
const (
	...
	responsesStreamErrorInputLimit   = 16 * 1024
	maxResponsesInitialPreambleBytes = 1024 * 1024
	maxResponsesPendingFrameBytes    = 1024 * 1024
)

var (
	responsesStreamCookieHeaderPattern         = regexp.MustCompile(`(?i)((?:set-)?cookie\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s;,"']+(?:\s*;\s*[A-Za-z0-9_.~-]+(?:=[^\s;,"']+)?)*)`)
	responsesStreamQuotedSensitiveValuePattern = regexp.MustCompile(`(?i)((?:"?(?:(?:x[-_])?(?:goog[-_])?api[\s_.-]?key|(?:access|refresh|session|id)[\s_.-]?token|token|key|(?:proxy[-_])?authorization|(?:client[-_])?secret|password|cookie)"?)\s*[=:]\s*)(["'])([^"']*)(["'])`)
	responsesStreamSensitiveValuePattern       = regexp.MustCompile(`(?i)((?:"?(?:(?:x[-_])?(?:goog[-_])?api[\s_.-]?key|(?:access|refresh|session|id)[\s_.-]?token|token|key|(?:proxy[-_])?authorization|(?:client[-_])?secret|password|cookie)"?)\s*[=:]\s*"?)([^\s"'&,;}]+)`)
	responsesStreamAuthorizationPattern        = regexp.MustCompile(`(?i)\b(?:Bearer|Basic)\s+[A-Za-z0-9._~+/=-]+`)
	responsesStreamURLUserinfoPattern          = regexp.MustCompile(`(?i)(https?://[^/\s:@]+:)([^@\s/]+)(@)`)
	responsesStreamTokenPattern                = regexp.MustCompile(`\b(?:sk-[A-Za-z0-9_-]{12,}|AIza[0-9A-Za-z_-]{20,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|xox[baprs]-[A-Za-z0-9-]{12,})\b`)
)
...
func redactResponsesStreamErrorText(text string) string {
	if len(text) > responsesStreamErrorInputLimit {
		text = text[:responsesStreamErrorInputLimit]
		for !utf8.ValidString(text) {
			text = text[:len(text)-1]
		}
		if quote := strings.LastIndexAny(text, `"'`); quote >= 0 {
			text += string(text[quote])
		}
	}
	text = responsesStreamCookieHeaderPattern.ReplaceAllString(text, `${1}[REDACTED]`)
	text = responsesStreamAuthorizationPattern.ReplaceAllStringFunc(text, func(value string) string {
		return strings.Fields(value)[0] + " [REDACTED]"
	})
	text = responsesStreamURLUserinfoPattern.ReplaceAllString(text, `${1}[REDACTED]${3}`)
	text = responsesStreamQuotedSensitiveValuePattern.ReplaceAllString(text, `${1}${2}[REDACTED]${4}`)
	text = responsesStreamSensitiveValuePattern.ReplaceAllString(text, `${1}[REDACTED]`)
	return responsesStreamTokenPattern.ReplaceAllString(text, "[REDACTED]")
}
...
func responsesStreamErrorText(errMsg *interfaces.ErrorMessage, status int) string {
	...
	if errMsg != nil && errMsg.Error != nil {
		candidate := redactResponsesStreamErrorText(errMsg.Error.Error())
