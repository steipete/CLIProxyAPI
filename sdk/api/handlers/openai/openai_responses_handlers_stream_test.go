...
func TestResponsesSSEFramerEmitsCompleteDataOnlyFramesAfterStart(t *testing.T) {
	var output bytes.Buffer
	framer := &responsesSSEFramer{}
	framer.WriteChunk(&output, []byte("event: response.created\ndata: {\"type\":\"response.created\"}\n\n"))
	framer.WriteChunk(&output, []byte(`data: {"type":"response.output_text.delta","delta":"ready"}`))
	if len(framer.pending) != 0 || !strings.Contains(output.String(), `"delta":"ready"`) {
		t.Fatalf("complete post-start data-only frame was retained: pending=%d output=%q", len(framer.pending), output.String())
	}
}

func TestResponsesSSEFramerRejectsOversizedPendingFrameAfterStart(t *testing.T) {
	var output bytes.Buffer
	framer := &responsesSSEFramer{failureEvent: "response.failed"}
	framer.WriteChunk(&output, []byte("event: response.created\ndata: {\"type\":\"response.created\"}\n\n"))
	framer.WriteChunk(&output, []byte(`data: {"type":"response.output_text.delta","delta":"`+strings.Repeat("x", maxResponsesPendingFrameBytes)))
	if framer.terminalError == nil || framer.terminalEvent != "response.failed" {
		t.Fatalf("oversized pending frame was accepted: error=%v event=%q", framer.terminalError, framer.terminalEvent)
	}
	if len(framer.pending) != 0 || !strings.Contains(output.String(), "maximum pending size") {
		t.Fatalf("oversized pending frame was retained or not reported: pending=%d output=%q", len(framer.pending), output.String())
	}
}
