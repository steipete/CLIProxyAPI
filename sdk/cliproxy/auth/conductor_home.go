...
const (
	...
	maxHomeRemoteRequestRetry         = 16
)

func boundedHomeRequestRetry(limit int) int {
	if limit > maxHomeRemoteRequestRetry {
		return maxHomeRemoteRequestRetry
	}
	if limit < 0 {
		return 0
	}
...
func (m *Manager) pickHomeDispatchSelection(ctx context.Context, model string, opts cliproxyexecutor.Options) (*HomeDispatchSelection, error) {
	...
	if pinnedAuthID == "" && dispatch.RequestRetry != nil && *dispatch.RequestRetry >= 0 {
		selection.requestRetry = boundedHomeRequestRetry(*dispatch.RequestRetry)
