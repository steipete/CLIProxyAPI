...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	if lastErr != nil {
		...
		if errors.As(lastErr, &bootstrapErr) && bootstrapErr != nil {
			return streamErrorResult(bootstrapErr.Headers(), lastErr, bootstrapErr.UpstreamAccepted()), nil
		}
		return nil, lastErr
	}
...
var (
	authLogCookieHeaderPattern         = regexp.MustCompile(`(?i)((?:set-)?cookie\s*[:=]\s*)(?:"[^"]*"|'[^']*'|[^\s;,"']+(?:\s*;\s*[A-Za-z0-9_.~-]+(?:=[^\s;,"']+)?)*)`)
	authLogQuotedSensitiveValuePattern = regexp.MustCompile(`(?i)((?:"?(?:(?:x[-_])?(?:goog[-_])?api[\s_.-]?key|(?:access|refresh|session|id)[\s_.-]?token|token|key|(?:proxy[-_])?authorization|(?:client[-_])?secret|password|cookie)"?)\s*[=:]\s*)(["'])([^"']*)(["'])`)
	authLogSensitiveValuePattern       = regexp.MustCompile(`(?i)((?:"?(?:(?:x[-_])?(?:goog[-_])?api[\s_.-]?key|(?:access|refresh|session|id)[\s_.-]?token|token|key|(?:proxy[-_])?authorization|(?:client[-_])?secret|password|cookie)"?)\s*[=:]\s*"?)([^\s"'&,;}]+)`)
	authLogAuthorizationPattern        = regexp.MustCompile(`(?i)\b(?:Bearer|Basic)\s+[A-Za-z0-9._~+/=-]+`)
	authLogURLUserinfoPattern          = regexp.MustCompile(`(?i)(https?://[^/\s:@]+:)([^@\s/]+)(@)`)
	authLogTokenPattern                = regexp.MustCompile(`\b(?:sk-[A-Za-z0-9_-]{12,}|AIza[0-9A-Za-z_-]{20,}|ghp_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]{20,}|xox[baprs]-[A-Za-z0-9-]{12,})\b`)
)

const authLogErrorInputLimit = 16 * 1024
...
func summarizeErrorForLog(err error) string {
	...
	msg := err.Error()
	if len(msg) > authLogErrorInputLimit {
		msg = msg[:authLogErrorInputLimit]
		for !utf8.ValidString(msg) {
			msg = msg[:len(msg)-1]
		}
		if quote := strings.LastIndexAny(msg, `"'`); quote >= 0 {
			msg += string(msg[quote])
		}
	}
	msg = strings.TrimSpace(msg)
	msg = authLogCookieHeaderPattern.ReplaceAllString(msg, `${1}[REDACTED]`)
	msg = authLogAuthorizationPattern.ReplaceAllStringFunc(msg, func(value string) string {
		return strings.Fields(value)[0] + " [REDACTED]"
	})
	msg = authLogURLUserinfoPattern.ReplaceAllString(msg, `${1}[REDACTED]${3}`)
	msg = authLogQuotedSensitiveValuePattern.ReplaceAllString(msg, `${1}${2}[REDACTED]${4}`)
	msg = authLogSensitiveValuePattern.ReplaceAllString(msg, `${1}[REDACTED]`)
	msg = authLogTokenPattern.ReplaceAllString(msg, "[REDACTED]")
	const maxRunes = 300
	runes := []rune(msg)
