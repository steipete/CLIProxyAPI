...
func (m *Manager) executeStreamWithModelPool(ctx context.Context, executor ProviderExecutor, auth *Auth, provider string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, routeModel, executionModel string, execModels []string, pooled bool, aliasResult OAuthModelAliasResult, allowRetry bool, ephemeralResult bool) (*cliproxyexecutor.StreamResult, error) {
	...
	for idx, execModel := range execModels {
		...
		// Each attempt gets its own cancelable context so an abandoned attempt
		// (refresh retry, bootstrap failure) can release its upstream stream
		// without canceling the caller's context.
		...
		// Fast-path: the executor reports the upstream accepted the stream, so
		// skip bootstrap buffering entirely and hand chunks straight through.
		if streamResult.UpstreamAccepted {
			return m.wrapStreamResult(ctx, cancelAttempt, auth.Clone(), provider, resultModel, true, streamResult.Headers, nil, streamResult.Chunks, aliasResult, ephemeralResult), nil
		}
		...
		return m.wrapStreamResult(ctx, cancelAttempt, auth.Clone(), provider, resultModel, streamResult.UpstreamAccepted, streamResult.Headers, buffered, remaining, aliasResult, ephemeralResult), nil
	}
	if lastErr == nil {
