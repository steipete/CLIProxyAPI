...
func TestWarnLogUpstreamFailureRedactsCredentialsInError(t *testing.T) {
	hook := setupTestLoggerHook(t)
	secrets := []string{
		"query-secret-value-123456",
		"header-secret-value-123456",
		"bearer-secret-value-123456",
		"sk-proj-abcdefghijklmnopqrstuvwxyz1234567890",
		"url-password-secret-123456",
		"YmFzaWMtc2VjcmV0LXZhbHVlLTEyMzQ1Ng==",
	}
	err := fmt.Errorf("POST https://user:%s@example.test/v1/responses?key=%s: x-api-key: %s Authorization: Bearer %s Proxy-Authorization: Basic %s upstream token %s", secrets[4], secrets[0], secrets[1], secrets[2], secrets[5], secrets[3])

	warnLogUpstreamFailure(context.Background(), nil, "openai", "gpt-4o", nil, time.Second, err)
	...
	for _, secret := range secrets {
		if strings.Contains(message, secret) {
			t.Fatalf("warning leaked credential %q: %q", secret, message)
		}
	}
	if !strings.Contains(message, "example.test/v1/responses") || !strings.Contains(message, "[REDACTED]") {
		t.Fatalf("warning discarded useful diagnostics or omitted redactions: %q", message)
	}
}

func TestWarnLogUpstreamFailureRedactsEntireQuotedCredentials(t *testing.T) {
	hook := setupTestLoggerHook(t)
	tests := []struct {
		name   string
		format string
		...
		{name: "double quoted api key", format: `api_key="%s"`, secret: "first-secret middle-secret last-secret"},
		{name: "single quoted access token", format: `access_token='%s'`, secret: "first-token middle-token last-token"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			hook.Reset()
			warnLogUpstreamFailure(context.Background(), nil, "openai", "gpt-4o", nil, time.Second, fmt.Errorf(tt.format, tt.secret))
			entries := hook.AllEntries()
			if len(entries) != 1 {
				t.Fatalf("warning count = %d, want 1", len(entries))
			}
			for _, part := range strings.Fields(tt.secret) {
				if strings.Contains(entries[0].Message, part) {
					t.Fatalf("warning leaked part of a quoted credential: %q", entries[0].Message)
				}
			}
		})
	}
...
func TestSummarizeErrorForLogRedactsCompleteCookieHeaders(t *testing.T) {
	for _, header := range []string{
		...
		got := summarizeErrorForLog(errors.New(header))
...
func TestSummarizeErrorForLogBoundsInputAndClosesTruncatedQuotedSecret(t *testing.T) {
	secretPrefix := "first-secret middle-secret last-secret"
	raw := `api_key="` + secretPrefix + strings.Repeat(" confidential", authLogErrorInputLimit) + `" trailing`
	got := summarizeErrorForLog(errors.New(raw))
	for _, part := range strings.Fields(secretPrefix) {
		if strings.Contains(got, part) {
			t.Fatalf("quoted credential leaked at truncation boundary: %q", got)
		}
	}
	if len(got) > 303 || !strings.Contains(got, "[REDACTED]") {
		t.Fatalf("bounded error summary = %q", got)
	}
}

func TestAuthCoolingSummaryRedactsAndBoundsUnavailableReason(t *testing.T) {
	now := time.Now()
	auth := &Auth{
		ID: "cooling-auth", Provider: "openai", FileName: "cooling.json",
		StatusMessage: "Cookie: session=first-cookie-secret; csrf=second-cookie-secret X-Trace: preserved " + strings.Repeat("x", 500),
	}
	got := authCoolingSummary(auth, "gpt-4o", now.Add(time.Minute), now)
	if strings.Contains(got, "first-cookie-secret") || strings.Contains(got, "second-cookie-secret") {
		t.Fatalf("cooldown reason leaked credentials: %q", got)
	}
	if !strings.Contains(got, "X-Trace: preserved") || !strings.Contains(got, "[REDACTED]") {
		t.Fatalf("cooldown reason lost useful context or omitted redaction: %q", got)
	}
	if len(got) > 400 {
		t.Fatalf("cooldown reason was not bounded: %d bytes", len(got))
	}
}
