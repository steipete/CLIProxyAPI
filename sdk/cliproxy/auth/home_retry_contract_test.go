...
func TestHomeRetryLimitsClampRemoteDispatchCredentialAndCooldown(t *testing.T) {
	huge := int(^uint(0) >> 1)
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{Home: internalconfig.HomeConfig{Enabled: true}})
	manager.SetRetryConfig(0, time.Second, 0)
	selectionLimit := -1
	manager.observeHomeRetryLimit(nil, &HomeDispatchSelection{requestRetry: huge, hasRequestRetry: true}, &selectionLimit)
	if selectionLimit != maxHomeRemoteRequestRetry {
		t.Fatalf("remote dispatch retry limit = %d, want %d", selectionLimit, maxHomeRemoteRequestRetry)
	}
	credentialLimit := -1
	manager.observeHomeRetryLimit(&Auth{Metadata: map[string]any{"request_retry": huge}}, nil, &credentialLimit)
	if credentialLimit != maxHomeRemoteRequestRetry {
		t.Fatalf("remote credential retry limit = %d, want %d", credentialLimit, maxHomeRemoteRequestRetry)
	}
	decoded := decodeHomeDispatchError([]byte(fmt.Sprintf(`{"error":{"type":"model_cooldown","message":"cooling","retry_after_ms":1,"request_retry":%d}}`, huge)))
	var cooldown *homeDispatchRetryAfterError
	if !errors.As(decoded, &cooldown) || cooldown == nil {
		t.Fatalf("decode cooldown error = %T %v", decoded, decoded)
	}
	if got, ok := cooldown.RequestRetryLimit(); !ok || got != maxHomeRemoteRequestRetry {
		t.Fatalf("decoded cooldown retry limit = (%d,%t), want (%d,true)", got, ok, maxHomeRemoteRequestRetry)
	}
	unsafeCooldown := &homeDispatchRetryAfterError{requestRetry: huge, hasRequestRetry: true}
	cooldownLimit := -1
	observeHomeCooldownRetryLimit(unsafeCooldown, &cooldownLimit, true)
	if cooldownLimit != maxHomeRemoteRequestRetry || manager.homeRetryAllowed(maxHomeRemoteRequestRetry, huge) {
		t.Fatalf("remote cooldown retry limit escaped final safety bound: limit=%d", cooldownLimit)
	}
}

func TestHomeExecutionClampsOversizedRemoteDispatchRetryRounds(t *testing.T) {
	huge := int(^uint(0) >> 1)
	dispatcher := &retryContractHomeDispatcher{authIDs: []string{"home-retry-a"}, requestRetry: &huge}
	executor := &retryContractHomeExecutor{failAll: true, failure: &Error{HTTPStatus: http.StatusBadGateway, Message: "upstream unavailable"}}
	manager := NewManager(nil, nil, nil)
	manager.SetConfig(&internalconfig.Config{Home: internalconfig.HomeConfig{Enabled: true}})
	manager.SetRetryConfig(0, time.Second, 1)
	manager.PublishHomeDispatch(dispatcher, executionregistry.New(), 1)
	manager.RegisterExecutor(executor)
	if _, err := manager.Execute(context.Background(), []string{"home-retry-contract"}, cliproxyexecutor.Request{Model: "gpt-4o"}, cliproxyexecutor.Options{}); err == nil {
		t.Fatal("expected upstream failure after bounded Home retry rounds")
	}
	if got := len(executor.Calls()); got != maxHomeRemoteRequestRetry+1 {
		t.Fatalf("Home execution attempts = %d, want %d", got, maxHomeRemoteRequestRetry+1)
	}
}
