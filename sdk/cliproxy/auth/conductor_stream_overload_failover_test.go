...
func TestStreamResultOptionsDropsOriginalRequestAndPreservesSessionAffinity(t *testing.T) {
	metadata := map[string]any{
		cliproxyexecutor.SessionAffinityProviderMetadataKey: "codex",
		cliproxyexecutor.SessionAffinityModelMetadataKey:    "gpt-4o",
		"custom": "preserved",
	}
	opts := cliproxyexecutor.Options{
		OriginalRequest: []byte(`{"prompt_cache_key":"primary-session","conversation":{"id":"fallback-session"},"input":"large request"}`),
		Metadata:        metadata,
	}
	resultOpts := streamResultOptions(opts)
	if resultOpts.OriginalRequest != nil {
		t.Fatalf("stream result retained %d request bytes", len(resultOpts.OriginalRequest))
	}
	if got := resultOpts.Metadata[streamResultPrimarySessionMetadataKey]; got != "pck:primary-session" {
		t.Fatalf("preserved primary session = %v", got)
	}
	if got := resultOpts.Metadata[streamResultFallbackSessionMetadataKey]; got != "conv:fallback-session" {
		t.Fatalf("preserved fallback session = %v", got)
	}
	if _, mutated := metadata[streamResultPrimarySessionMetadataKey]; mutated {
		t.Fatal("result session metadata mutated the executor-owned metadata map")
	}
	selector := NewSessionAffinitySelector(&RoundRobinSelector{})
	defer selector.Stop()
	for _, session := range []string{"pck:primary-session", "conv:fallback-session"} {
		selector.cache.Set("codex::"+session+"::gpt-4o", "sticky-auth")
	}
	selector.OnResult(Result{AuthID: "sticky-auth", Provider: "codex", Model: "gpt-4o", Success: false, Options: resultOpts})
	for _, session := range []string{"pck:primary-session", "conv:fallback-session"} {
		if got, ok := selector.cache.GetAndRefresh("codex::" + session + "::gpt-4o"); ok {
			t.Fatalf("failed credential retained session affinity %q = %q", session, got)
		}
	}
}
