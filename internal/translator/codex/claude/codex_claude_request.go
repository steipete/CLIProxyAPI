...
// Codex Responses rejects smaller output budgets, including one-token health probes.
const codexMinimumMaxOutputTokens = 16
...
func ConvertClaudeRequestToCodex(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	if maxTokens := rootResult.Get("max_tokens"); maxTokens.Exists() {
		...
		if maxOutputTokens < codexMinimumMaxOutputTokens {
			maxOutputTokens = codexMinimumMaxOutputTokens
		}
		template, _ = sjson.SetBytes(template, "max_output_tokens", maxOutputTokens)
	}
