...
func TestConvertClaudeRequestToCodex_ClampsCodexOutputTokenMinimum(t *testing.T) {
	tests := []struct {
		name      string
		maxTokens int
		want      int64
	}{
		{name: "one-token health probe", maxTokens: 1, want: 16},
		{name: "below minimum", maxTokens: 15, want: 16},
		{name: "at minimum", maxTokens: 16, want: 16},
		{name: "ordinary request", maxTokens: 4096, want: 4096},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			input := []byte(fmt.Sprintf(`{"model":"claude-fable-5","max_tokens":%d,"messages":[{"role":"user","content":"hello"}]}`, tt.maxTokens))
			result := gjson.ParseBytes(ConvertClaudeRequestToCodex("gpt-5.6-sol", input, false))
			if got := result.Get("max_output_tokens").Int(); got != tt.want {
				t.Fatalf("max_output_tokens = %d, want %d: %s", got, tt.want, result.Raw)
			}
		})
	}
}
