...
func buildResponsesInterruptedEvent(st *oaiToResponsesState, requestRawJSON []byte, nextSeq func() int) []byte {
	failed := []byte(`{"type":"response.failed","sequence_number":0,"response":{"id":"","object":"response","created_at":0,"status":"failed","error":{"code":"incomplete_stream","message":"upstream stream ended before completing the response"}}}`)
	...
	if model := gjson.GetBytes(requestRawJSON, "model"); model.Exists() {
		failed, _ = sjson.SetBytes(failed, "response.model", model.String())
	}
...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if isDone {
		...
		if hasActiveUnfinishedTool || len(st.MsgItemAdded) == 0 && len(st.FuncItemAdded) == 0 && st.FinishReason == "" {
			st.CompletedEmitted = true
			out = append(out, buildResponsesInterruptedEvent(st, requestForNamespace, nextSeq))
			return out
		}
		st.CompletedEmitted = true
