...
func TestConvertOpenAIChatCompletionsResponseToOpenAIResponses_InterruptedStreamsEmitTerminalFailure(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "reasoning only",
			chunk: `data: {"id":"resp_interrupted_reasoning","object":"chat.completion.chunk","created":1773896263,"model":"gpt-4o","choices":[{"index":0,"delta":{"role":"assistant","reasoning_content":"still thinking"},"finish_reason":null}]}`,
		},
		{
			name:  "tool without arguments",
			chunk: `data: {"id":"resp_interrupted_tool","object":"chat.completion.chunk","created":1773896263,"model":"gpt-4o","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_patch","type":"function","function":{"name":"apply_patch","arguments":""}}]},"finish_reason":null}]}`,
		},
		{
			name:  "tool with partial arguments",
			chunk: `data: {"id":"resp_interrupted_partial","object":"chat.completion.chunk","created":1773896263,"model":"gpt-4o","choices":[{"index":0,"delta":{"tool_calls":[{"index":0,"id":"call_patch","type":"function","function":{"name":"apply_patch","arguments":"{\"filePath\":\"foo"}}]},"finish_reason":null}]}`,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var param any
			terminalFailures := 0
			for _, line := range []string{tt.chunk, "data: [DONE]", "data: [DONE]"} {
				for _, chunk := range ConvertOpenAIChatCompletionsResponseToOpenAIResponses(context.Background(), "gpt-4o", request, request, []byte(line), &param) {
					event, data := parseOpenAIResponsesSSEEvent(t, chunk)
					...
					if got := data.Get("response.status").String(); got != "failed" {
						t.Fatalf("terminal response.status = %q, want failed", got)
					}
					if got := data.Get("response.error.code").String(); got != "incomplete_stream" {
						t.Fatalf("terminal response.error.code = %q, want incomplete_stream", got)
					}
				}
			}
			if terminalFailures != 1 {
				t.Fatalf("interrupted stream emitted %d terminal failures, want 1", terminalFailures)
			}
		})
	}
}

func TestConvertOpenAIChatCompletionsResponseToOpenAIResponses_EmptyFinishedStreamsEmitExpectedTerminal(t *testing.T) {
	request := []byte(`{"model":"gpt-4o"}`)
	for _, tt := range []struct {
		...
		{reason: "stop", event: "response.completed", status: "completed"},
		{reason: "length", event: "response.incomplete", status: "incomplete"},
		{reason: "content_filter", event: "response.incomplete", status: "incomplete"},
	} {
		t.Run(tt.reason, func(t *testing.T) {
			var param any
			terminalCount := 0
			line := `data: {"id":"resp_empty","object":"chat.completion.chunk","created":1773896263,"model":"gpt-4o","choices":[{"index":0,"delta":{},"finish_reason":"` + tt.reason + `"}]}`
			for _, input := range []string{line, "data: [DONE]"} {
				for _, chunk := range ConvertOpenAIChatCompletionsResponseToOpenAIResponses(context.Background(), "gpt-4o", request, request, []byte(input), &param) {
					event, data := parseOpenAIResponsesSSEEvent(t, chunk)
					if event == "response.failed" {
						t.Fatalf("explicitly finished empty stream failed: %s", chunk)
					}
					if event == tt.event {
						terminalCount++
						if got := data.Get("response.status").String(); got != tt.status {
							t.Fatalf("response.status = %q, want %q", got, tt.status)
						}
					}
				}
			}
			if terminalCount != 1 {
				t.Fatalf("terminal %s count = %d, want 1", tt.event, terminalCount)
			}
		})
	}
}
