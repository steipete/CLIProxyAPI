...
func TestHandleDirectWebsocketRejectsClientSecretClientFacingModelEscalation(t *testing.T) {
	gin.SetMode(gin.TestMode)
	handler := NewHandler(auth.NewManager(nil, nil, nil), nil)
	router := gin.New()
	router.GET("/v1/realtime", func(c *gin.Context) {
		c.Set(ClientSecretSessionContextKey, json.RawMessage(`{"type":"realtime","model":"gpt-realtime"}`))
		c.Set(ClientSecretPrincipalContextKey, "sess_scoped")
		c.Set(ClientSecretModelContextKey, "gpt-realtime-mini")
		c.Next()
	}, handler.HandleRealtimeWebsocket)
	...
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, request)
	if recorder.Code != http.StatusForbidden || !strings.Contains(recorder.Body.String(), "realtime_client_secret_scope_mismatch") {
		t.Fatalf("mini-scoped secret accessed full model: status=%d body=%s", recorder.Code, recorder.Body.String())
	}
}
