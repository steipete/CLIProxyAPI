...
func TestCreateClientSecretMapsStandardRealtimeModel(t *testing.T) {
	...
	if authorization.ClientModel != "gpt-realtime" {
		t.Fatalf("client model scope = %q, want gpt-realtime", authorization.ClientModel)
	}
...
func TestClientSecretStoreBoundsAnonymousIssuerWithoutExhaustingGlobalCapacity(t *testing.T) {
	store := newClientSecretStore()
	session := json.RawMessage(`{"type":"realtime","model":"gpt-realtime"}`)
	for i := 0; i < clientSecretMaxEntriesPerIssuer; i++ {
		if _, _, _, err := store.create(session, time.Minute, "", ""); err != nil {
			t.Fatalf("anonymous secret %d: %v", i, err)
		}
	}
	if _, _, _, err := store.create(session, time.Minute, "", "alternate-provider"); !errors.Is(err, errClientSecretCapacity) {
		t.Fatalf("anonymous overflow error = %v, want client secret capacity", err)
	}
	if _, _, _, err := store.create(session, time.Minute, "authenticated-issuer", "static"); err != nil {
		t.Fatalf("anonymous issuance exhausted authenticated capacity: %v", err)
	}
}
