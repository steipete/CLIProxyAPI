...
const (
	...
	ClientSecretModelContextKey     = "codexLiveClientSecretModel"
...
func (s *clientSecretStore) create(session json.RawMessage, lifetime time.Duration, issuerPrincipal, issuerProvider string, clientModels ...string) (string, ClientSecretAuthorization, time.Time, error) {
	if s == nil {
	...
	if len(clientModels) > 0 {
		authorization.ClientModel = strings.TrimSpace(clientModels[0])
	}
	now := s.currentTime()
	...
	for _, entry := range s.entries {
		if entry.authorization.IssuerPrincipal == authorization.IssuerPrincipal &&
			(authorization.IssuerPrincipal == "" || entry.authorization.IssuerProvider == authorization.IssuerProvider) {
			issuerEntries++
		}
	}
...
func (h *Handler) createClientSecret(c *gin.Context, session json.RawMessage, expiresAfter *struct {
	...
	token, authorization, expiresAt, errCreate := h.clientSecrets.create(upstreamSession, lifetime, issuerPrincipalValue, issuerProviderValue, modelFromJSON(clientSession))
	if errCreate != nil {
