...
func TestRealtimeClientSecretCannotEscalateClientFacingModel(t *testing.T) {
	server := newTestServer(t)
	create := httptest.NewRequest(http.MethodPost, "/v1/realtime/client_secrets", strings.NewReader(`{"session":{"type":"realtime","model":"gpt-realtime-mini"}}`))
	create.Header.Set("Authorization", "Bearer test-key")
	created := httptest.NewRecorder()
	server.engine.ServeHTTP(created, create)
	if created.Code != http.StatusOK {
		t.Fatalf("create scoped secret: status=%d body=%s", created.Code, created.Body.String())
	}
	var response struct {
		Value string `json:"value"`
	}
	if err := json.Unmarshal(created.Body.Bytes(), &response); err != nil || response.Value == "" {
		t.Fatalf("decode scoped secret: value=%q err=%v", response.Value, err)
	}
	upgrade := httptest.NewRequest(http.MethodGet, "/v1/realtime?model=gpt-realtime", nil)
	upgrade.Header.Set("Authorization", "Bearer "+response.Value)
	upgrade.Header.Set("Connection", "Upgrade")
	upgrade.Header.Set("Upgrade", "websocket")
	upgraded := httptest.NewRecorder()
	server.engine.ServeHTTP(upgraded, upgrade)
	if upgraded.Code != http.StatusForbidden || !strings.Contains(upgraded.Body.String(), "realtime_client_secret_scope_mismatch") {
		t.Fatalf("mini-scoped secret accessed full model: status=%d body=%s", upgraded.Code, upgraded.Body.String())
	}
}
