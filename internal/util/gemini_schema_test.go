...
func TestInlineLocalRefsBoundsAcyclicSharedDefinitionExpansion(t *testing.T) {
	definitions := make(map[string]any)
	definitions["level0"] = map[string]any{"type": "string"}
	for level := 1; level <= 12; level++ {
		previousRef := fmt.Sprintf("#/definitions/level%d", level-1)
		definitions[fmt.Sprintf("level%d", level)] = map[string]any{
			"type": "object",
			"properties": map[string]any{
				"left":  map[string]any{"$ref": previousRef},
				"right": map[string]any{"$ref": previousRef},
			},
		}
	}
	input, err := json.Marshal(map[string]any{
		...
		"type":        "object",
		"properties": map[string]any{
			"payload": map[string]any{"$ref": "#/definitions/level12"},
		},
	})
	if err != nil {
	...
	output := inlineLocalRefs(string(input))
	if len(output) > 256*1024 {
		t.Fatalf("acyclic shared references expanded without a practical bound: input=%d output=%d", len(input), len(output))
	}
	if !strings.Contains(output, `"description":"See: level`) {
		t.Fatalf("expansion budget did not fall back to typed reference hints: %s", output)
	}
	if got := gjson.Get(output, "properties.payload.type").String(); got != "object" {
		t.Fatalf("root reference lost its schema type: %q", got)
	}
}

func TestInlineLocalRefsPreservesPropertiesNamedDefinitions(t *testing.T) {
	input := `{"definitions":{"Payload":{"type":"string"}},"type":"object","properties":{"definitions":{"type":"object","properties":{"nested":{"$ref":"#/definitions/Payload"}}}}}`
	result := gjson.Parse(inlineLocalRefs(input))
	if got := result.Get("properties.definitions.properties.nested.type").String(); got != "string" {
		t.Fatalf("property named definitions lost local-reference expansion: %s", result.Raw)
	}
}

func TestInlineLocalRefsBoundsRepeatedLargeTargetExpansion(t *testing.T) {
	properties := make(map[string]any)
	for index := 0; index < 64; index++ {
		properties[fmt.Sprintf("copy%d", index)] = map[string]any{"$ref": "#/definitions/Payload"}
	}
	input, err := json.Marshal(map[string]any{
		"definitions": map[string]any{"Payload": map[string]any{
			"type":        "object",
	...
	if len(output) > 2*1024*1024 {
		t.Fatalf("large shared target amplified without a cumulative size budget: input=%d output=%d", len(input), len(output))
	}
	if !strings.Contains(output, `"description":"See: Payload"`) {
		t.Fatal("large-target expansion did not fall back to a typed reference hint")
	}
}
