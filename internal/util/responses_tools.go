...
func BuildGeminiFunctionDeclarations(root gjson.Result) ([][]byte, map[string]string, map[string]ResponsesToolIdentity) {
	...
	emittedNames := make(map[string]struct{}, len(sanitizedMap))
	for _, emittedName := range sanitizedMap {
		emittedNames[emittedName] = struct{}{}
	}
	...
	for _, desc := range winningList {
		...
		if desc.Name != geminiName {
			if _, emitted := emittedNames[desc.Name]; !emitted {
				if _, exists := reverseMap[desc.Name]; !exists {
