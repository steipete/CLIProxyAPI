...
func TestBuildGeminiFunctionDeclarations_EmittedNameBeatsCollidingOriginalAlias(t *testing.T) {
	collidingAlias := sanitizeResponsesToolNames([]string{"read/file", "read_file"})["read/file"]
	raw := fmt.Sprintf(`{"tools":[{"type":"function","name":"read/file"},{"type":"function","name":"read_file"},{"type":"custom","name":%q}]}`, collidingAlias)
	_, forward, reverse := BuildGeminiFunctionDeclarations(gjson.Parse(raw))
	emitted := forward["read/file"]
	if emitted != collidingAlias {
		t.Fatalf("test did not produce an emitted/original collision: emitted=%q alias=%q", emitted, collidingAlias)
	}
	if identity := reverse[emitted]; identity.Name != "read/file" || identity.Custom {
		t.Fatalf("original-name alias overwrote authoritative emitted identity: %+v", identity)
	}
	if identity := reverse[forward[collidingAlias]]; identity.Name != collidingAlias || !identity.Custom {
		t.Fatalf("colliding tool lost its own authoritative emitted identity: %+v", identity)
	}
}
