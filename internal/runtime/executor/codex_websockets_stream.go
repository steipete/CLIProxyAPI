...
func (e *CodexWebsocketsExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	body = applyCodexSourceRequestCompatibility(body, from, auth)
	...
	if buffering {
		for {
			if ctx != nil && ctx.Err() != nil {
				if sess != nil {
					e.invalidateUpstreamConnWithoutDisconnectNotify(sess, conn, "request_canceled", ctx.Err())
					sess.clearActive(conn, readCh)
					unlockStreamSession()
				} else {
					_ = closer.Close()
			...
			if isCodexHandshakeMetadataEvent(eventType) && !isTerminalEvent {
				chunkBytes := codexBootstrapChunkBytes(currentChunks)
				if bufferedHandshakeEvents < codexBootstrapMaxBufferedEvents && chunkBytes <= codexBootstrapMaxBufferedBytes-bufferedBytes {
					bufferedChunks = append(bufferedChunks, currentChunks...)
					bufferedBytes += chunkBytes
					bufferedHandshakeEvents++
					continue
				}
				helps.LogWithRequestID(ctx).Debugf("codex websockets executor: bootstrap buffer limit reached (%d events, %d bytes), releasing stream without overload probing", bufferedHandshakeEvents, bufferedBytes)
			}
