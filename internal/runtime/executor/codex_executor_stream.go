	...
	if buffering {
		for scanner.Scan() {
			...
			if isHandshake && !terminalSuccess {
				chunkBytes := codexBootstrapChunkBytes(chunks)
				...
				helps.LogWithRequestID(ctx).Debugf("codex executor: bootstrap buffer limit reached (%d events, %d bytes), releasing stream without overload probing", bufferedHandshakeEvents, bufferedBytes)
			}
