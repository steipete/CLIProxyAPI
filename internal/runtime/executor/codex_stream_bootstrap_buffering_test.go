...
func TestCodexExecutor_BootstrapBuffering_ByteLimitReleasesStream(t *testing.T) {
	largeHandshake := fmt.Sprintf(`{"type":"response.created","response":{"id":"resp_large","padding":"%s"}}`, strings.Repeat("x", codexBootstrapMaxBufferedBytes))
	server := codexSSEServer(largeHandshake, codexOverloadEvent)
	defer server.Close()
	req, opts := codexTestRequest()
	result, err := NewCodexExecutor(codexBufferingConfig(true)).ExecuteStream(context.Background(), codexTestAuth(server.URL), req, opts)
	if err != nil || result == nil {
		t.Fatalf("oversized bootstrap handshake was retained for overload failover: result=%v err=%v", result, err)
	}
	if _, streamErr := drainChunks(result); streamErr == nil {
		t.Fatal("overload after the bootstrap byte limit must remain an in-stream failure")
	}
}

func TestCodexExecutor_BootstrapBuffering_CountsSuppressedHandshakeEvents(t *testing.T) {
	events := make([]string, 0, codexBootstrapMaxBufferedEvents+2)
	for i := 0; i <= codexBootstrapMaxBufferedEvents; i++ {
		events = append(events, fmt.Sprintf(`{"type":"response.created","response":{"id":"resp_%d"}}`, i))
	}
	events = append(events, codexOverloadEvent)
	server := codexSSEServer(events...)
	defer server.Close()
	...
	result, err := NewCodexExecutor(codexBufferingConfig(true)).ExecuteStream(context.Background(), codexTestAuth(server.URL), req, opts)
	if err != nil || result == nil {
		t.Fatalf("suppressed handshake events bypassed bootstrap limit: result=%v err=%v", result, err)
	}
	if _, streamErr := drainChunks(result); streamErr == nil {
		t.Fatal("overload after the suppressed-event limit must remain an in-stream failure")
	}
}
...
func TestCodexWebsocketsExecutor_BootstrapBuffering_ByteLimitReleasesStream(t *testing.T) {
	largeHandshake := fmt.Sprintf(`{"type":"response.created","response":{"id":"resp_large","padding":"%s"}}`, strings.Repeat("x", codexBootstrapMaxBufferedBytes))
	server := codexWebsocketServer(t, largeHandshake, codexOverloadEvent)
	defer server.Close()
	req, opts := codexWebsocketRequest()
	result, err := NewCodexWebsocketsExecutor(codexBufferingConfig(true)).ExecuteStream(context.Background(), codexTestAuth(server.URL), req, opts)
	if err != nil || result == nil {
		t.Fatalf("oversized websocket bootstrap handshake was retained for overload failover: result=%v err=%v", result, err)
	}
	if _, streamErr := drainChunks(result); streamErr == nil {
		t.Fatal("overload after the websocket bootstrap byte limit must remain an in-stream failure")
	}
}

func TestCodexWebsocketsExecutor_BootstrapBuffering_CountsSuppressedHandshakeEvents(t *testing.T) {
	events := make([]string, 0, codexBootstrapMaxBufferedEvents+2)
	for i := 0; i <= codexBootstrapMaxBufferedEvents; i++ {
		events = append(events, fmt.Sprintf(`{"type":"response.created","response":{"id":"resp_%d"}}`, i))
	}
	...
	result, err := NewCodexWebsocketsExecutor(codexBufferingConfig(true)).ExecuteStream(context.Background(), codexTestAuth(server.URL), req, opts)
	if err != nil || result == nil {
		t.Fatalf("suppressed websocket handshake events bypassed bootstrap limit: result=%v err=%v", result, err)
	}
	if _, streamErr := drainChunks(result); streamErr == nil {
		t.Fatal("overload after the suppressed websocket-event limit must remain an in-stream failure")
	}
}
...
type cancelOnWebsocketActivationContext struct {
	context.Context
...
func (c *cancelOnWebsocketActivationContext) Err() error {
	c.mu.Lock()
	defer c.mu.Unlock()
	if !c.canceled {
		c.session.activeMu.Lock()
		c.canceled = c.session.activeConn != nil
		c.session.activeMu.Unlock()
	}
	if c.canceled {
...
func TestCodexWebsocketsExecutor_BootstrapBuffering_CanceledSessionInvalidatesConnection(t *testing.T) {
	server := codexWebsocketServerHoldingConnection(t)
	defer server.Close()

	exec := NewCodexWebsocketsExecutor(codexBufferingConfig(true))
	exec.store = &codexWebsocketSessionStore{sessions: make(map[string]*codexWebsocketSession)}
	const sessionID = "canceled-bootstrap-session"
	sess := exec.getOrCreateSession(sessionID)
	ctx := &cancelOnWebsocketActivationContext{Context: context.Background(), session: sess}
	...
	sess.connMu.Unlock()
	if conn != nil {
		t.Fatal("canceled bootstrap left its upstream websocket reusable for the next session turn")
	}
}
