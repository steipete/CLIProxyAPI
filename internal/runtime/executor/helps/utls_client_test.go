...
func TestUtlsRoundTripperHandshakeDeadlineClosesOwnedConnection(t *testing.T) {
	clientConn, serverConn := net.Pipe()
	t.Cleanup(func() {
		_ = clientConn.Close()
		_ = serverConn.Close()
	...
	h2Conn, errConnect := roundTripper.createConnection(ctx, "example.test", "example.test:443")
	if h2Conn != nil {
		_ = h2Conn.Close()
		t.Fatal("unexpected HTTP/2 connection after handshake deadline")
	}
	if !errors.Is(errConnect, context.DeadlineExceeded) {
		t.Fatalf("createConnection error = %v, want deadline exceeded", errConnect)
	}
	if got := trackedConn.closeCount.Load(); got != 1 {
		t.Fatalf("owned connection close count = %d, want 1", got)
	}
}
