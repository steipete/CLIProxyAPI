...
func TestAntigravityTransportCacheBoundsCredentialPoolConnectionBudget(t *testing.T) {
	if antigravityTransportCacheCapacity > 128 {
		t.Fatalf("credential transport cache capacity = %d, want at most 128", antigravityTransportCacheCapacity)
	}
	if budget := antigravityTransportCacheCapacity * antigravityMaxIdleConnsPerHost; budget > 12800 {
		t.Fatalf("credential pool idle connection budget = %d, want at most 12800", budget)
	}
	original := antigravityTransports
	...
	for i := 0; i <= antigravityTransportCacheCapacity; i++ {
		auth := antigravityAuthWithIDAndProxy(fmt.Sprintf("bounded-auth-%d", i), "")
		_ = newAntigravityHTTPClient(context.Background(), &config.Config{}, auth, 0)
	}
	if got := antigravityTransports.Len(); got != antigravityTransportCacheCapacity {
		t.Fatalf("cached credential pools = %d, want %d", got, antigravityTransportCacheCapacity)
	}
}
